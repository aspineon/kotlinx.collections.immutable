...
internal class PersistentVectorBuilder<E>(private var vector: PersistentList<E>,
    ...
    override fun addAll(elements: Collection<E>): Boolean {
        ...
        if (MAX_BUFFER_SIZE - tailSize >= elements.size) {
            ...
            @Suppress("UNCHECKED_CAST")
            root = addBuffers(root, rootSize(), buffers as Array<Array<Any?>>)
     ...
     */
    private fun addBuffers(root: Array<Any?>?, rootSize: Int, buffers: Array<Array<Any?>>): Array<Any?> {
        ...
        var mutableRoot = when {
            rootSize shr LOG_MAX_BUFFER_SIZE < 1 shl rootShift ->
                ...
                placeBuffers(root, rootSize, rootShift, buffersIterator)
            else ->
        ...
        while (buffersIterator.hasNext()) {
            ...
            placeBuffers(mutableRoot, 1 shl rootShift, rootShift, buffersIterator)
        }
    ...
    /**
     * Places buffers from the [buffersIterator] as the leaf nodes.
     * In the result of this call [root] node gets entirely filled, or all buffers get placed.
     *
     * Returns the resulting root.
     */
    private fun placeBuffers(root: Array<Any?>?, rootSize: Int, shift: Int, buffersIterator: Iterator<Array<Any?>>): Array<Any?> {
        ...
        @Suppress("UNCHECKED_CAST")
        mutableRoot[index] =
                placeBuffers(mutableRoot[index] as Array<Any?>?, rootSize, shift - LOG_MAX_BUFFER_SIZE, buffersIterator)

        while (++index < MAX_BUFFER_SIZE && buffersIterator.hasNext()) {
            @Suppress("UNCHECKED_CAST")
            mutableRoot[index] =
                    placeBuffers(mutableRoot[index] as Array<Any?>?, 0, shift - LOG_MAX_BUFFER_SIZE, buffersIterator)
        }
        return mutableRoot
    }
    ...
    override fun addAll(index: Int, elements: Collection<E>): Boolean {
        ...
        @Suppress("UNCHECKED_CAST")
        root = addBuffers(root, unaffectedElementsCount, buffers as Array<Array<Any?>>)
        tail = newTail
        size += elements.size
    ...
    override fun removeAll(elements: Collection<E>): Boolean {
        ...
        return removeAllWithPredicate { hashSet.contains(it) }
    }
    ...
    fun removeAllWithPredicate(predicate: (E) -> Boolean): Boolean {
        ...
        // build the root
        val newRoot = if (buffers.isEmpty()) root!! else placeBuffers(root, unaffectedElementsCount, rootShift, buffers.iterator())
